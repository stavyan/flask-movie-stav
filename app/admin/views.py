...
from app.models import Admin, Tag, Movie, Preview, User, Comment, Moviecol, Oplog, Adminlog, Userlog, Auth, Role, Admin
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if movie_count == 1 and movie.title != data['title']:
            flash("片名已经存在！", "err")
            return redirect(url_for('admin.movie_eidt', id=id))
        if not os.path.exists(app.config["UP_DIR"]):
            os.makedirs(app.config["UP_DIR"])
            os.chmod(app.config["UP_DIR"], "rw")
...
def adminloginlog_list(page=None):
    ...
    return render_template("admin/adminloginlog_list.html", pagedata=page_data)
